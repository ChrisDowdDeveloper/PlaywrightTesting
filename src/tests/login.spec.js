...
test.describe('Valid scenarios', () => {
    ...
    */
    ...
    test('user can be redirected to the create account page from the login page', async () => {
        ...
        await loginPage.createRedirect();
    ...
    */
    test.only('user can login with valid credentials', async () => {
    ...
    test('users email is remembered after logging out', async() => {
        ...
        const isRemembered = await loginPage.rememberUsername();

        expect(isRemembered).toBe(process.env.EBAY_EMAIL);
    });
